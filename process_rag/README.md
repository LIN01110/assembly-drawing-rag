...
## 大规模合成集评测（2026-09-09：2500 合成用例 ×3 臂，--no-llm 可复现）

用例由 `eval/gen_cases.py` 从 30 张历史工艺卡做**参数域内扰动**合成（车间归属不变保证派工 GT 合法，seed=42）：

| 记忆库臂 | 派工正确率 | 合规率 | 记忆库 Recall@3 | 归档率 |
|---|---|---|---|---|
| cbr | 64.1% | 86.1% | 100% | 86.1% |
| vector | 60.6% | 86.1% | 100% | 86.1% |
| hybrid | 60.6% | 86.1% | 100% | 86.1% |

**合成集显著难于人工精选集**（派工 64.1% vs 76.0%）：随机扰动落在规则边界附近的样本暴露了派工规则的简化假设——这正是扩大评测集的价值；13.9% 的草案被质量门隔离待人工，隔离机制在规模化输入下持续生效。
