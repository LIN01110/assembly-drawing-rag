...
运行：python eval/evaluate.py [--no-llm]
      默认 2500 合成用例（eval/cases_synth.json）；
      --cases-file eval/cases.json 可切回 25 个人工精选用例快速验证
"""
import argparse
import json
import os
...
CASES_PATH = Path(__file__).parent / "cases_synth.json"
...
def main() -> None:
    ...
    ap.add_argument("--no-llm", action="store_true", help="全程模板降级，零 API（可复现）")
    ap.add_argument("--llm-hybrid-only", action="store_true",
                    help="仅 hybrid 臂用 LLM，其余两臂 --no-llm（混合模式）")
    ap.add_argument("--cases-file", default=str(CASES_PATH),
                    help="用例文件（默认 eval/cases_synth.json，2500 合成用例）")
